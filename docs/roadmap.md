...
1. **Cookie primitives + auth interoperability** — explicit cookie helpers on the native foundation; Better Auth as an integration recipe, never a first-party auth system.
...
The framework implementation, compatibility matrix, clean-room review, package rehearsal, and candidate attestation are complete for the `v0.1.0-beta.1` line, regenerated through the M7-GATE (`ae4e29f`) and M8-GATE (`3edaae9`) release integrations. **`lugas@0.1.0-beta.2` is published under npm `beta`** (source commit `7f08b16`, npm README logo fix); `latest` remains `0.1.0-beta.1` pending an owner dist-tag action. The owner's re-attestation decision ([#330](https://github.com/ther12k/lugas/issues/330)) keeps the `2ed954d` set preserved unmodified as superseded history under [`releases/superseded/2ed954d/`](./releases/superseded/2ed954d/). npm publication, dist-tag moves, and new release candidates remain explicitly owner-controlled.
