...
The framework implementation, compatibility matrix, clean-room review, package rehearsal, and candidate attestation are complete for the `v0.1.0-beta.1` line. The attested artifact set under [`releases/beta/`](./releases/beta/) was regenerated through the M7-GATE (`ae4e29f`) and M8-GATE (`3edaae9`) release integrations and now carries the four M8 first-party batteries (CORS, SSE, structured logging, OpenAPI/Scalar); the owner's re-attestation decision ([#330](https://github.com/ther12k/lugas/issues/330)) keeps the `2ed954d` set preserved unmodified as superseded history under [`releases/superseded/2ed954d/`](./releases/superseded/2ed954d/). Publication remains an explicitly owner-controlled action.
