...
The framework implementation, compatibility matrix, clean-room review, package rehearsal, and candidate attestation are complete through the M7-GATE (`ae4e29f`), M8-GATE (`3edaae9`), and beta.3 release integration (`f3c72e6`). **`lugas@0.1.0-beta.2` is published under npm `beta`** (source commit `7f08b16`, npm README logo fix); `latest` remains `0.1.0-beta.1` pending an owner dist-tag action. The **`v0.1.0-beta.3` candidate** (Drizzle, M9-001) is fully attested in [`releases/beta/`](releases/beta/RELEASE_PACKET.md) — tarball `11d7033e…`, gate PASS with zero blocking failures — pending owner publication. The owner's re-attestation decision ([#330](https://github.com/ther12k/lugas/issues/330)) keeps the `2ed954d` set preserved unmodified as superseded history under [`releases/superseded/2ed954d/`](./releases/superseded/2ed954d/). npm publication, dist-tag moves, and new release candidates remain explicitly owner-controlled.
