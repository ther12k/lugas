...
Accepted by owner decision (ODR-0008, `docs/owner-decisions/m8-002-dispatch.md`, 2026-09-09), dispatching issue [#349](https://github.com/ther12k/lugas/issues/349) (M8-002) as the second milestone-8 battery. Fulfills the roadmap row "Server-Sent Events — planned core helper". The roadmap gate on lifecycle evidence is satisfied by M7-004 ([ADR-0020](0020-application-service-lifecycle.md)). **Amended during M8-002 implementation (same issue scope, recorded in the evidence report):** probing showed Bun surfaces a mid-stream error on a zero-byte response as a silent `200` with an empty body, so decision point 4 was strengthened — `start()` runs synchronously before any wire bytes and a throw surfaces through the route error policy as a redacted `500` Problem Details; the header-flush behavior (first byte) was recorded in decision point 5.
...
4. **Failure semantics:** invalid `sse()` configuration and invalid writer input throw stable diagnostics (`LUGAS_SSE_001`, `LUGAS_SSE_002`) at the public boundary — never bare `TypeError`s. `start` runs **synchronously inside `sse()`, before any wire bytes**, so a `start` that throws propagates to the route's error policy as an ordinary handler failure — a redacted `500` Problem Details response — never a silent empty-200 (pinned by probe: Bun surfaces a mid-stream error on a zero-byte response as `200` with an empty body). Lugas invents no error-frame protocol.
5. **Primitive, not product:** no event broker, no per-client fan-out, no persistence/replay, no `EventSource` polyfill in `lugas/client` (the browser consumes SSE natively). `last-event-id` is an ordinary request header applications read themselves. Fan-out architectures compose by holding writers in application-owned services. Response headers flush with the first written byte: producers should open with an initial event, `retry`, or comment rather than sitting idle (HTTP-inherent in Bun's streaming response path).
