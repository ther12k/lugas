...
## 2026-09-04 — M6D1 documentation restructure

- External review of the 683-line README (main@7edb841) implemented: README cut to a 194-line public front door (logo, badges, reviewer's section order), detailed content moved to new docs pages (`getting-started`, `wire-honest-types`, `design-principles`, `roadmap`), examples indexed (`examples/README.md` incl. proof-api; `examples/client/README.md` added; `docs/examples.md` corrected), community files added (issue templates, CODE_OF_CONDUCT, SUPPORT, CHANGELOG, rewritten CONTRIBUTING).
- Astro+Starlight documentation site added under `website/` (Astro 7.3.1, Starlight 0.42.0; builds 13 pages at base `/lugas`); `docs/` stays canonical via a deterministic `sync-docs.ts` that rewrites links and injects site frontmatter. New `.github/workflows/docs-site.yml` (new file inside the protected workflows path, owner-directed) builds and deploys on pushes to main; Pages enablement is the owner step.
- Release integrity honored: `docs/releases/beta/` artifacts untouched; new `POST-FREEZE-NOTES.md` records that the tarball's npm README is the frozen `2ed954d` version and the owner must publish unchanged or cut a new candidate and re-attest — never rebuild under the same attestation.
- `scripts/generate-llms.ts` updated (stale TSX purpose phrase removed, RESPONSE diagnostics added, key files refreshed) and `llms.txt` regenerated; `llms-full.txt` regenerated byte-identical. Evidence: `docs/reports/issues/M6D1.md`.

## 2026-09-04 — M6R13 guard short-circuit + enrichment typing

- #328 (external review, beta-release-blocker class): `GuardContribution` now extracts enrichment via `Exclude<Awaited<Result>, Response>` — a guard returning `TypedResponse<401, …> | { user }` (sync, Promise, or mixed) contributes exactly the enrichment to the handler context instead of a `Record<never, never>`-poisoned union; response-only guards contribute nothing. Compile-time only; `src/core/types.ts` extraction helpers were verified already union-correct (guard 401/403 statuses already reached the client status union) and are now pinned by probes.
- The primary auth pattern typechecks end to end: `examples/auth/app.ts` lost all three `ctx: any` annotations; the README quick-start guard/client flow compiles against the real public API through new verbatim snippet fixtures (`tests/docs/fixtures/`, drift-guarded by `tests/docs/readme-snippets.test.ts`) — the Starlight site cannot typecheck fences, so the fixture set is the type gate for the public quick-start.
- Same-review polish: README guard snippet made self-contained (full imports, `export default`, matching `currency` schema), duplicate `# LugasJS` H1 removed, docs-site workflow Bun pinned via `.bun-version`.
- Verify: 653 pass / 1 skip / 0 fail (113 files), typecheck clean, docs clean. The attested `2ed954d` artifact is untouched; publication remains publish-as-is vs new candidate + re-attestation, an explicit owner decision. Evidence: `docs/reports/issues/M6R13.md`.
