---
type: Guide
title: Guards and Typed Context
status: current
tags:
- guide
- guards
...
- **Not auth product**: Lugas ships no JWT/OAuth/session machinery — the guard contract is the integration point (see the non-goals in [`design-principles.md`](./design-principles.md)).
- **Not for static assets**: asset responses bypass the pipeline entirely.

## Where next

- [Validation](./validation.md) — the slots validated before your guards see the context.
- [Responses](./responses.md) — typed short-circuit responses and the error policy.
- [Testing](./testing.md) — asserting guard order through a real server.
