...
| RF-3 | **Framework-owned statuses (422, 413, 415, 400) are absent from the typed-client status union.** The union is handler-return + guard short-circuits only, so `invalid.status === 422` is a compile error even though every validation-failed response is documented wire behavior. A route declaring `body` could statically know "422 possible". | API-shaped | **resolved:** 422/415/400 for schema-declared routes (CA-5, PR #408); multipart 415/400/413 branches + end-to-end `formBody()` client uploads for `form()` routes (CA-6, `docs/reports/issues/CA-6.md`); the bare transport-ceiling 413 stays out of every union by design — see `docs/client.md` |
