...
| RF-3 | **Framework-owned statuses (422, 413, 415, 400) are absent from the typed-client status union.** The union is handler-return + guard short-circuits only, so `invalid.status === 422` is a compile error even though every validation-failed response is documented wire behavior. A route declaring `body` could statically know "422 possible". | API-shaped | **resolved for schema-declared routes:** 422/415/400 join the union (CA-5, `docs/reports/issues/CA-5.md`); 413 stays out by design (budget is runtime config; the transport 413 is bare — see `docs/client.md`); `form()` branches arrive with multipart client support |
