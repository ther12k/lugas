...
`lugasLifecycle.ready` gates on every `init`; `shutdown()` is idempotent and reports `connectionsClosed`, `trackedWorkCompleted`, `disposalCompleted`, and `disposalFailures` distinctly. See [services](./services.md) for the phase semantics.
...
- [Services](./services.md) — lifecycle semantics being asserted above.
- [Diagnostics](./diagnostics.md) — what each `LUGAS_*` code means when a test surfaces one.
- [`examples/`](../examples/README.md) — runnable single-concept apps to copy from.
