...
The 429 short-circuit is built with the typed response helpers, so the [typed client](client.md) discriminates it precisely: a rate-limited call yields `{ ok: false, status: 429, error }` in the outcome union — no `{ status: number }` widening. For generated documents, declare the response in the route's `openapi` metadata (see [`docs/openapi.md`](openapi.md)) so consumers see the 429 contract; how the client classifies the body is specified in [`docs/client-error-semantics.md`](client-error-semantics.md).
