...
# CORS Policy (First-Party, ADR-0022)
...
([ADR-0004](okf/decisions/0004-bun-native-router-authoritative.md)) stays the
...
- static `Response` values and `Bun.file()`/`Blob` values in any route
  position;
- native `{ dir }` mounts;
- `assets` configuration ([ADR-0018](okf/decisions/0018-opt-in-public-asset-serving.md)).
