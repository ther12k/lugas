---
title: "Telemetry"
description: "Dependency-free request events and the OpenTelemetry span recipe."
---
...
**Redaction by construction** — the [logging](/lugas/logging/) field discipline extended: headers, bodies, cookies, params, and query never appear in events. An authorization header, a session cookie, a query token: none of them can leak, structurally.

## The end boundary includes tracked work

`request.end` fires when the response is produced **and** any work the request registered through `track()` has settled — the same boundary the graceful drain waits on ([services](/lugas/services/)). Detached work is correlated by passing the request itself as the token:
...
- [Logging](/lugas/logging/) — the access log and request-id facility telemetry composes with.
- [Services](/lugas/services/) — the drain boundary that defines request-end.
- [Diagnostics](/lugas/diagnostics/) — `LUGAS_TELEMETRY_001`.
