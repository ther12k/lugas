---
title: "WebSockets"
description: "Guard-gated upgrades on Bun native sockets with shutdown close-1001."
---
...
`before` guards run before the handshake. A guard returning a `Response` rejects the upgrade with that exact response — authentication and authorization are ordinary guards, identical to [routes](/lugas/routing/):
...
- **No CORS on handshakes.** The WebSocket handshake is not subject to browser CORS, so the first-party [CORS](/lugas/cors/) policy cannot protect it. **Check the `Origin` header in a guard** for browser-facing sockets:
...
New upgrades are refused once shutdown begins (the server stops accepting), and the rest of the lifecycle — drain deadline, reverse-order disposal, distinct outcomes — behaves exactly as documented in [services](/lugas/services/).
...
- [Guards](/lugas/guards/) — the enrichment that flows into your socket handlers.
- [SSE](/lugas/sse/) — the one-directional companion for server-push streams.
- [Services](/lugas/services/) — the shutdown lifecycle closing your sockets.
