...
description: "Route maps, path syntax, modules, and the derived handler context."
---
...
Params arrive on `ctx.params` as strings (Bun's wire truth). Declaring a `params` schema transforms and retypes them — see [validation](/lugas/validation/).
...
| `before` | `GuardDescriptor[]` | Ordered guards. See [guards](/lugas/guards/). |
...
- `name` must be a non-empty, unique string within the app (`LUGAS_APP_005` on duplicates); it appears in the [manifest](/lugas/manifest-v1/).
...
To type `ctx.services`, name the services shape as `route()`'s first generic — usually inferred from `defineApp({ services })` when routes are declared inside the app config. See [services](/lugas/services/).
...
Or in-process: `app.manifest` is the frozen `lugas-manifest-v1` document. See [manifest-v1](/lugas/manifest-v1/).

## Where next

- [Validation](/lugas/validation/) — the four schema slots and the `422` failure contract.
- [Guards](/lugas/guards/) — ordered guards and typed context enrichment.
- [Responses](/lugas/responses/) — typed response helpers and the error policy.
