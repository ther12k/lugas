---
title: "Validation"
description: "Standard Schema slots, coercion, and the 422 failure contract."
---
...
Lugas feature-detects `~standard.jsonSchema` and never guesses shapes from vendor names. A validator without a JSON Schema representation documents **presence only** in [OpenAPI](/lugas/openapi/) (e.g. a query parameter marked required, a body typed `object`) — structure is never invented. Validators that expose JSON Schema have it used verbatim.
...
Mixing validators across routes is fine — each slot is independent. The compatibility matrix records verified combinations in [`compatibility.md`](/lugas/compatibility/).

## Where next

- [Routing](/lugas/routing/) — where schemas attach and what the route map accepts.
- [Guards](/lugas/guards/) — enrichment runs after validation in the compiled pipeline order.
- [Responses](/lugas/responses/) — the `422` envelope and the rest of the error policy.
