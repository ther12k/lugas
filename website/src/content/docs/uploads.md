...
`body: form(...)` occupies the ordinary body slot: the manifest records the body capability, `ctx.body` is typed from the descriptor, and every framework error path applies (Problem Details, redaction, [logging](/lugas/logging/), [CORS](/lugas/cors/), [secure headers](/lugas/production/)).

## Bounded consumption

The battery's point: uploads are the heaviest request class, so they are held to the [body budget](https://github.com/ther12k/lugas/blob/main/docs/body-limits.md) like every framework-parsed body:
...
- **Repeated part names collapse last-wins**, mirroring [`parseCookies`](/lugas/cookies/). The native `request.formData()` remains available in handlers when you genuinely need multiplicity.
...
- [Validation](/lugas/validation/) — Standard Schema slots for the JSON lanes (compose: validate `ctx.body.fields` in the handler, or parse uploads to JSON first).
- [Diagnostics](/lugas/diagnostics/) — `LUGAS_FORM_001` and the failure codes above.
