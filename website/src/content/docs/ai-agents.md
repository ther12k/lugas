...
description: "Agent-readable surfaces, LLM streaming recipe, MCP proposal."
---
...
- **Client side**: `EventSource` is GET-only, so for `POST /chat` consume the response stream with `fetch` (`response.body.getReader()`), or expose the route as GET with the prompt in the query. The [typed client](/lugas/client/) deliberately does not wrap streams.
- **Keys stay server-side**: the upstream authorization header is set in the handler; clients see only your SSE frames.
- **CORS applies**: `/chat` is an ordinary pipeline route, so [`defineApp({ cors })`](/lugas/cors/) governs browser access — token endpoints are exactly where you want the fail-closed default.
